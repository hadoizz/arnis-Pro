...
pub fn start_map_preview_generation(info: MapPreviewInfo) {
    ...
    std::thread::spawn(move || {
        // 2D PNG only — emit as soon as it exists so the GUI modal can open without waiting on 3D mesh.
        let png_result = std::panic::catch_unwind(std::panic::AssertUnwindSafe(|| {
        ...
        match png_result {
            Ok(Ok(_path)) => {
                ...
                // 3D mesh in a detached thread so it can overlap with UI work and doesn’t extend the
                // preview thread longer than necessary.
                let wp = info.world_path.clone();
                let min_x = info.min_x;
                let max_x = info.max_x;
                let min_z = info.min_z;
                let max_z = info.max_z;
                std::thread::spawn(move || {
                    let _ = std::panic::catch_unwind(std::panic::AssertUnwindSafe(|| {
                        match preview_mesh::generate_preview_mesh_gzip(&wp, min_x, max_x, min_z, max_z)
                        {
                            Ok(gz) => {
                                let mesh_path = wp.join("arnis_preview_mesh.bin.gz");
                                if let Err(e) = fs::write(&mesh_path, gz) {
                                    eprintln!("Warning: Failed to write 3D preview mesh: {e}");
                                }
                            }
                            Err(e) => {
                                eprintln!("Warning: 3D preview mesh not generated: {e}");
                            }
                        }
                    }));
                });
